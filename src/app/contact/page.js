...
export default function ContactPage() {
  ...
  useEffect(() => {
    const loadDistrict = async () => {
      ...
      try {
        const snap = await getDoc(
          doc(db, "websites", "biohaloscom", "districts", currentDistrict)
        );
        if (snap.exists()) {
                ...
                {loading ? (
                  <div className="space-y-4">
                    {[...Array(3)].map((_, i) => (
                      <div
                        key={i}
                        className="h-28 rounded-3xl bg-slate-200/60 animate-pulse"
                      />
                    ))}
                  </div>
                  ...
                  contactInfo.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex items-start gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-xs transition-all hover:border-slate-300 hover:shadow-md"
                      >
                        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-slate-900 shrink-0">
                          {getFieldIcon(label)}
                        </div>
                              ...
                              if (isPhone) {
                                return (
                                  <a
                                    ...
                                    className="text-base sm:text-lg font-bold text-slate-900 hover:text-slate-700 transition-colors inline-block"
                                  >
                                    {val}
                                  </a>
                                );
                              ...
                              if (isEmail) {
                                return (
                                  <a
                                    key={vIdx}
                                    href={`mailto:${val}`}
                                    className="text-base font-bold text-slate-900 hover:text-slate-700 transition-colors inline-block break-all"
                                  >
                                    {val}
                                  </a>
                                );
          ...
          <div className="mt-12 space-y-4">
            {faqs.map((faq, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-3xl border border-slate-200 bg-slate-50 overflow-hidden transition-all"
                >
                  <button
                  ...
                  >
                    ...
                    <ChevronDown
                      size={20}
                      className={`shrink-0 text-slate-900 transition-transform duration-300 ${isOpen ? "rotate-180" : ""
                        }`}
                    />
                  </button>
